...
// Forked the timing code from the CPW engine.
// Forked the UCI code from an old engine I built
    ...
    for (i=0;i<ct;i++) {
        ...
        if ((MOVE_FROM(move)==from) && (MOVE_TO(move)==to)) {
            if (!isalpha(s[4])) {return move;}
                ...
                if (IS_PROMOTION(move) && (PROMOTE_TO(move) == promoted)) {return move;}
            }
        }
    }
    return NOMOVE;
